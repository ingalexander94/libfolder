...
const ListPartsContent = () => {
  ...
  const [idItemInventory, setIdItemInvetory] = useState<number>(0);
    ...
    <div className={styles.table_system}>
          ...
          ? (inventoryState.list_inventory.map((item, i) => (
            <div key={i} className={styles.table_item}>
              <ul>
                ...
                <li>
                  <strong>{item.inventory_quantity ?? "$0"}</strong>
                </li>
                <li>
                  <strong>{item.inventory_units ?? "$0"}</strong>
                </li>
                <li>
                  <strong>{item.inventory_price_without_tax ?? "$0"}</strong>
                </li>
                <li>
...

      {
        ...
        (<AlertDelete 
          ...
          to_delete={async() => {
            try {
              const res = await callEndpoint(InventoryService.delete(idItemInventory));
      ...
      {modalVisible && (
        <AddPart
          title={"Editar"}
          id_item_inventory={idItemInventory}
          closeModal={() => {
            getInventory();
            setIdItemInvetory(0);
